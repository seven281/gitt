...
#pragma pack(8)
//任务堆栈
CPU_STK stackBufferTaskServiceLiquidAuto[STK_SIZE_TASK_SERVICE_LIQUID_AUTO];
#pragma pack()

OS_TCB tcbTaskServiceLiquidAuto;


//串口指令处理
static void TaskServiceCommandProc_LiquidAuto_UartRS1(SYSTEM_CMD_UART_RS1* uartCommand);

//CAN口指令处理
static void TaskServiceCommandProc_LiquidAuto_Can1(SYSTEM_CMD_CAN1* canCommand);

//网口指令处理
static void TaskServiceCommandProc_LiquidAuto_NetW5500(SYSTEM_CMD_NET_W5500* netCommand);

//自身指令处理
static void TaskServiceCommandProc_LiquidAuto_Self(SYSTEM_CMD_SELF* selfCommand);
    ...
    while(1)
    {
        ...
        if ((taskSystemRecvCommandPtr != NULL) && (err == OS_ERR_NONE))
        {
            ...
            if (SYSTEM_CMD_SRC_UART_RS1 == taskSystemRecvCommandPtr->commandSrc)
            {
                ...
                //对接收到的数据进行数据处理
                TaskServiceCommandProc_LiquidAuto_UartRS1(uartRS1CommandPtr);
                //处置完成,执行后续操作
            ...
            else if (SYSTEM_CMD_SRC_SELF == taskSystemRecvCommandPtr->commandSrc)
            {
                ...
                //对接收到的数据进行处理
                TaskServiceCommandProc_LiquidAuto_Self(selfCommandPtr);
                //处置完成,执行后续操作
            ...
            else if(SYSTEM_CMD_SRC_NET_W5500 == taskSystemRecvCommandPtr->commandSrc)
            {
                ...
                //对接收到的数据进行处理
                TaskServiceCommandProc_LiquidAuto_NetW5500(netW5500CommandPtr);
                //处置完成,执行后续操作
            ...
            else if(SYSTEM_CMD_SRC_CAN1 == taskSystemRecvCommandPtr->commandSrc)
            {
                ...
                //对接收到的数据进行处理
                TaskServiceCommandProc_LiquidAuto_Can1(can1CommandPtr);
                //处置完成,执行后续操作
