...
//低于该值的中断将不会被屏蔽,操作系统不会认识到该中断的发生.

//也就是说,低于该值的中断是快速中断,他不会因为系统调度而被屏蔽,依然可以发生,这样可以防止丢中断,但是,不要在这些中断中调用
//系统相关的函数

//大于等于该值的中断是系统中断,系统会感知到这些中断的发生,所以也就可以调用系统API函数

//如果这个值设置的不对的话,中断时间次数多了,系统会出错

//因为系统的中断四位,所以,慎重起见,将中断感知级别设置为7 ,也就是抢占优先级1,子优先级3
#define  CPU_CFG_KA_IPL_BOUNDARY                           7u
