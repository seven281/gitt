 ...
 *@return	num:返回转化后的整型数
 */
uint32 atoi32(char *str, uint16_t base) {
  uint32 num = 0;
  while (*str != 0)
    num = num * base + c2d(*str++);
  return num;
 ...
 *@return	无
 */
void itoa(uint16_t n, uint8 str[5], uint8 len) {
...
char *inet_ntoa(unsigned long addr) {
  ...
  sprintf(addr_str, "%d.%d.%d.%d", (int)(addr >> 24 & 0xFF),
...
/**
 *@brief		将一个 主机模式的unsigned short型数据转换到大端模式的TCP/IP
 *网络字节格式的数据.
 *@param		要转换的数据
 *@return 	大端模式的数据
 */
...
/**
 *@brief		将一个 主机模式的unsigned long型数据转换到大端模式的TCP/IP
 *网络字节格式的数据.
 *@param		要转换的数据
 *@return 	大端模式的数据
 */
unsigned long htonl(unsigned long hostlong /**< hostshort  - A 32-bit number in
                                              host byte order.  */
) {
#if (SYSTEM_ENDIAN == _ENDIAN_LITTLE_)
...
/**
 *@brief		将一个大端模式的TCP/IP
 *网络字节格式的数据转换到主机模式的unsigned short型数据
 *@param		要转换的数据
 *@return 	unsigned short模式的数据
 */
unsigned long
ntohs(unsigned short netshort /**< netshort - network odering 16bit value */
) {
#if (SYSTEM_ENDIAN == _ENDIAN_LITTLE_)
...
/**
 *@brief		将一个大端模式的TCP/IP
 *网络字节格式的数据转换到主机模式的unsigned long型数据
 *@param		要转换的数据
...
u_char check_dest_in_local(u_long destip) {
  ...
  for (i = 0; i < 4; i++) {
    if ((pdestip[i] & IINCHIP_READ(SUBR0 + i)) !=
        (IINCHIP_READ(SIPR0 + i) & IINCHIP_READ(SUBR0 + i)))
      return 1; // Remote
  }
  return 0;
}
