...
/**
 *@brief		This Socket function initialize the channel in perticular
 *mode, and set the port and wait for W5200 done it.
 *@param		s: socket number.
 *@param		protocol: The socket to chose.
 *@param		port:The port to bind.
 *@param		flag: Set some bit of MR,such as **< No Delayed Ack(TCP)
 *flag.
 *@return  	1 for sucess else 0.
 */
uint8 socket(SOCKET s, uint8 protocol, uint16_t port, uint8 flag) {
  uint8 ret;
...
/**
 *@brief		This function close the socket and parameter is "s" which
 ...
 *@return  	1 for success else 0.
 */
uint8 connect(SOCKET s, uint8 *addr, uint16_t port) {
  uint8 ret;
 ...
 *@return  1 for success else 0.
 */
uint16_t send(SOCKET s, const uint8 *buf, uint16_t len) {
  uint8 status = 0;
  uint16_t ret = 0;
  uint16_t freesize = 0;
...
/**
 *@brief		This function is an application I/F function which is used
 *to receive the data in TCP mode. It continues to wait for data as much as the
 ...
 */
uint16_t recv(SOCKET s, uint8 *buf, uint16_t len) {
  uint16_t ret = 0;
  if (len > 0) {
...
/**
 *@brief		This function is an application I/F function which is used
 *to send the data for other then TCP mode. Unlike TCP transmission, The peer's
 ...
 */
uint16_t sendto(SOCKET s, const uint8 *buf, uint16_t len, uint8 *addr,
              uint16_t port) {
  uint16_t ret = 0;
      ...
      ((port == 0x00))) //||(ret == 0) )
  {
    ...
    while ((IINCHIP_READ(Sn_IR(s)) & Sn_IR_SEND_OK) != Sn_IR_SEND_OK) {
      if (IINCHIP_READ(Sn_IR(s)) & Sn_IR_TIMEOUT) {
        IINCHIP_WRITE(Sn_IR(s), (Sn_IR_SEND_OK |
                                 Sn_IR_TIMEOUT)); /*Clear SEND_OK & TIMEOUT*/
        return 0;
      }
    }
    IINCHIP_WRITE(Sn_IR(s), Sn_IR_SEND_OK);
...
/**
 *@brief		This function is an application I/F function which is used
 *to receive the data in other then TCP mode. This function is used to receive
 ...
 */
uint16_t recvfrom(SOCKET s, uint8 *buf, uint16_t len, uint8 *addr, uint16_t *port) {
  uint8 head[8];
  uint16_t data_len = 0;
  uint16_t ptr = 0;
  uint32 addrbsb = 0;
 ...
 */
uint16_t macraw_send(const uint8 *buf, uint16_t len) {
  uint16_t ret = 0;
  uint8 sock_num;
  sock_num = 0;
 ...
 *@return	This function return received data size for success else 0.
 */
uint16_t macraw_recv(uint8 *buf, uint16_t len) {
  uint8 sock_num;
  uint16_t data_len = 0;
  uint16_t dummyPort = 0;
  uint16_t ptr = 0;
  ...
  if (len > 0) {
    ...
    data_len = IINCHIP_READ_RXBUF(
        0, ptr); /*Read_data(s, (uint8 *)ptr, data, len) & read data*/
    ptr++;
    ...
    if (data_len > 1514) {
      ...
      close(sock_num); /*Close the 0-th socket*/
      socket(sock_num, Sn_MR_MACRAW, dummyPort,
